...
if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') == false && strpos($_SERVER['HTTP_USER_AGENT'], 'QQ/') == false) {
  //非微信浏览器打开 执行登陆判断
        if(!$_SESSION['uid'] && !$_GET['code']){
           $baseurl='http://fex.herooms.com.cn/connect/sign.php?sign='.$this->extract['xianliao_sign'].'&'.$_SERVER['QUERY_STRING'];
           $url='https://open.xianliao.updrips.com/connect/oauth2/authorize?appid='.$this->extract['xianliao_appid'].'&redirect_uri='.$baseurl.'&response_type=code&scope=snsapi_userinfo&state=123#xianliao_redirect';
           header("Location: ".$url);
           exit;
        }elseif($_GET['code']&& !$_SESSION['uid']){
            import('Common.Lib.xianliao');
            $this->weixin = new \weixin($this->extract[xianliao_appid],$this->extract[xianliao_key],$_GET['code']);
            ...
            if(!$user){
                    ...
                    $add=array();
                    $add['id']=$last['id']+87;
                    $add['maccountid']=$add['id'];
                    $add['user_login']='闲聊'.$add['id'];
                    $add['password']='jt';
                    $add['create_time']=$this->time;
                    $add['last_time']=$this->time;
                    $add['reg_ip']=get_client_ip(0, true);
                    // 解决canvas跨域绘图问题
                    if(!$userxx['originalAvatar']){$add['img']='https://merchant.xianliao.updrips.com/static/images/material/xianliao-logo-58-b1ec7fc80f.png';}else{$add['img']=$userxx['originalAvatar'];}
                    $add['nickname']=$userxx['nickName'];
                    $add['nickname_base64'] = base64_encode($userxx['nickName']);
                    $add['sex']=$userxx['gender'];
                    $add['openid']=$userxx['openId'];
                    $add['id_key']=md5($add['openid'].time());
...
}elseif(strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') == true){
  //微信浏览器打开 执行登陆判断
        if(!$_SESSION['uid'] && !$_GET['code']){
           $baseurl=urlencode('http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING']);
           $url='http://weixin.bi83f.cn/weixin.html?appid='.$this->extract['weixin_appid'].'&redirect_uri='.$baseurl.'&response_type=code&scope=snsapi_userinfo&state=123#wechat_redirect';
           header("Location: ".$url);
           exit;
        }elseif($_GET['code'] && !$_SESSION['uid']){ 
...
}elseif(strpos($_SERVER['HTTP_USER_AGENT'], 'QQ/') == true){
    if(!$_SESSION['uid'] && !$_GET['code']){
    $dialog_url = "https://graph.qq.com/oauth2.0/authorize?response_type=code&client_id=".$this->extract['qq_appid']."&redirect_uri=".urlencode($this->extract['qq_url'])."&state=".base64_encode($this->extract['game_url']);
    echo("<script> top.location.href='".$dialog_url."'</script>");
    exit;
        }elseif($_GET['code'] && !$_SESSION['uid']){
	        import('Common.Lib.qq');
            $this->qq = new \qq($this->extract[qq_appid],$this->extract[qq_key],$this->extract[qq_url],$_GET['code']);
            $result=$this->qq->get_oauth2($_GET['code']);
            $openid=$result['openid'];
            $map=array();
            $map['openid']=$openid;
            $user=M('user')->where($map)->find();
            if(!$user){
                    $userxx=$this->qq->get_userinfo($this->extract[qq_appid],$openid);
                    $last= M('user')->order('id desc')->find();
                    $add=array();
                    $add['id']=$last['id']+87;
                    $add['maccountid']=$add['id'];
                    $add['user_login']='QQ'.$add['id'];
                    $add['password']='jt';
                    $add['create_time']=$this->time;
                    $add['last_time']=$this->time;
                    $add['reg_ip']=get_client_ip(0, true);
                    // 解决canvas跨域绘图问题
                    $add['img']=$userxx['figureurl_qq_1'];
                    $add['nickname']=$userxx['nickname'];
                    $add['nickname_base64'] = base64_encode($userxx['nickname']);
                    if($userxx['gender']=="男"){$userxx['sex']='0';}else{$userxx['sex']='1';}
                    $add['sex']=$userxx['sex'];
                    $add['openid']=$userxx['openid'];
                    $add['id_key']=md5($add['openid'].time());
                    $add['yhid']=rand(2,3).rand(10000,99999);
        ...
        if(!$this->user){
           echo "No Token !  Accounts are blocked  By HeroHu";exit;
        }
        if(I('get.skin')){
            $save=array();
            $this->user['password']=$save['password']=base64_decode('anQ=');
